...
class LearningAgent(Agent):
    ...
    def createQ(self, state):
        ...
        state_str = str(state)
        if state_str not in self.Q.keys():
            self.Q[state_str] = {}
            for action in self.valid_actions:
                self.Q[state_str][action] = 0.0
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            learning = self.alpha*(reward-self.Q[state][action])
            self.Q[state][action] = self.Q[state][action] + learning
        return
...
def run():
    ...
    #sim = Simulator(env)
    sim = Simulator(env, update_delay=0.01, log_metrics=True, display=True, optimized=False)
